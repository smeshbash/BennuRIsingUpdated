...
const heroRegex = /<div className="max-w-6xl mx-auto relative z-10 clearfix pt-8 lg:pr-\[350px\]">([\s\S]*?)<\/div>\s*<\/div>\s*{\/\* Widget placement/m;
...
const newHeroContent = `<div className="max-w-6xl mx-auto relative z-10 pt-8 lg:pr-[350px]">
              <div className="flex flex-col lg:flex-row items-center lg:items-center gap-8 lg:gap-16">
                  {/* Logo Column */}
                  <div className="flex justify-center items-center relative flex-shrink-0 w-full lg:w-[40%]">
                     <div className="absolute w-full h-full bg-white/40 blur-[50px] rounded-full scale-75 -z-10 animate-pulse"></div>
                     <img src="/logo1.png" alt="Bennu Rising Logo" className="relative z-10 h-48 md:h-64 lg:h-auto lg:w-full max-w-sm drop-shadow-2xl animate-float filter contrast-110 object-contain" referrerPolicy="no-referrer" />
                  </div>
                  
                  {/* Hero Text Column */}
                  <div className="space-y-8 animate-fade-in-up flex flex-col items-center lg:items-start text-center lg:text-left flex-1 w-full">
